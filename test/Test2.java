...
public class Test2 {
    ...
    private static final int buf_size = 1024 * 1024; // 1KB
    ...
    public ByteBuffer allocate(int tid, int rand) { 
    ...
    static class T2Runnable implements Runnable {
        ...
        public void run() {
            ...
            for (;;) {
                ...
                lists.get(rand).add((new Test2()).allocate(tid, rand));
...
//                }
	...
	Date start = new Date();
        ...
        // TODO - if this cycle is commented, ByteBuffer code is not JITted using our code! 
//        for(int i = 0; i < 1024*1024; i++) { (new Test2()).allocate(-1, -1); }
